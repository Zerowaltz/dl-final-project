...
class AttentionMatrix(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        atten_matrix =  tf.matmul(Q, K, transpose_b=True)  / tf.math.sqrt(tf.cast(window_size_keys, dtype=tf.float32))
        if self.use_mask:
            atten_matrix = tf.add(atten_matrix, atten_mask)
...
class AttentionHead(tf.keras.layers.Layer):
    def __init__(self, input_size, output_size, is_self_attention, **kwargs):
        ...
        self.K = self.add_weight(shape=(input_size, output_size), initializer="glorot_uniform")
        self.V = self.add_weight(shape=(input_size, output_size), initializer="glorot_uniform")
        self.Q = self.add_weight(shape=(input_size, output_size), initializer="glorot_uniform")
    ...
    def call(self, inputs_for_keys, inputs_for_values, inputs_for_queries):
        ...
        K = tf.tensordot(inputs_for_keys, self.K, axes=(1)) #put axis
        V = tf.tensordot(inputs_for_values, self.V, axes=(1))
        Q = tf.tensordot(inputs_for_queries, self.Q, axes=(1))
        m = self.attention_matrix((K, Q)) ##wrap in tuple if doesnt work
...
class TransformerBlock(tf.keras.layers.Layer):
    def __init__(self, emb_sz, MultiHeadedAttentionB=False, **kwargs):
        ...
        self.layer_norm = tf.keras.layers.LayerNormalization(epsilon=0.001)
    ...
    def call(self, inputs, context_sequence):
        ...
        x2 = self.layer_norm(tf.add(x, inputs))
        y = self.self_context_atten(context_sequence, context_sequence, x2) #queries should be output from self attention
        y4 = self.layer_norm(tf.add(y, x2))
        y5 = self.ff_layer(y4)
        y6 = self.layer_norm(tf.add(y5, y4))
        y7 = tf.nn.relu(y6) 
...
class PositionalEncoding(tf.keras.layers.Layer):
    ...
    def __init__(self, vocab_size, embed_size, window_size):
        ...
        self.pos_encoding = positional_encoding(window_size, embed_size) ##vocab size?
    ...
    def call(self, x):
        ...
        x = tf.add(x, self.pos_encoding[tf.newaxis, :length, :])
        return x
